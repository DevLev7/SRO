...
<?php if (is_plugin_active('multycity/multycity.php')) {
	...
	if(!$title || $title == "Посмотри в SEO") {
		if($page_ID_current == $page_ID_contacts) {
			$title = 'Контакты «'.$brand.'» '.strip_tags(do_shortcode('[city_gde]'));
			$description = 'Контакты «'.$brand.'». Наш адрес: г. '.strip_tags(do_shortcode('[city_i]')).', '.do_shortcode('[city_adr]').'. Режим работы: '.get_field('mode', 'option').'. Звоните по номеру: '.do_shortcode('[city_ph]') ;
		}
...
<?php }	?>
	...
	<link rel="stylesheet" type="text/css" href="<?php echo THEME_CSS; ?>/swiper-bundle.min.css" />
